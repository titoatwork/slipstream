...
PagedAttention · continuous batching · chunked prefill · prefix caching · custom Triton kernels · speculative decoding (verified correct, not yet faster). A CUDA-graph pool exists and is micro-benchmarked, but is not yet in the serving loop. Tensor parallelism and prefill/decode disaggregation are planned for Phase 6 and exist only as stubs today.
